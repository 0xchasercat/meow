---
spec_id: LOAD-004
title: PnP bridge for Deno Node CommonJS
...
LOAD-004 now owns the bridge between meow's PnP package graph and Deno's Node resolver / require loader. Deno owns CommonJS execution. Meow owns which package/version/member is selected and where that package is exposed on disk.
...
- package contents are exposed through `meow_pkg::UnpackedStore` / equivalent immutable extracted cache paths, as `dir_for(package).join(member)`;
- no project `node_modules` is written or read by default.

This is the key design point: Deno's Node resolver is filesystem/path-oriented, so meow supplies stable real paths from the global unpacked store rather than forcing `meow-cache://` through Deno internals.
...
- `node_resolver::NpmPackageFolderResolver`
  - `resolve_package_folder_from_package(package_name, referrer)` returns the unpacked folder for the exact package/version selected by `ResolutionGraph` for this `referrer`.
  - `resolve_types_package_folder(...)` returns the matching `@types/*` folder when present in the graph, else `None` honestly.
- `node_resolver::InNpmPackageChecker`
  - true for URLs/paths inside the global unpacked package store used by the bridge;
  - false for first-party local files and meow-native modules.
- `deno_node::NodeRequireLoader`
  - reads text from local files or unpacked cache paths;
...
- `node_resolver::PackageJsonResolver` / `NodeResolverSys`
  - use a filesystem view that can read local project files and unpacked store package files.
...
- Cached packages expose stable global unpacked store paths.
- `meow-cache://` remains allowed for diagnostics/internal identity, but the Deno Node compatibility layer uses real paths.
- No fabricated `node_modules/...` path is reported unless `--materialize` is explicitly in use.

### A7 — LSP/runtime parity is graph-selection parity
After Amendment 003, parity means runtime, LSP, and node tooling select the same package/version/member from the same `ResolutionGraph`. Deno may own the detailed Node resolver behavior inside the runtime bridge. `.meow/deps` is a tooling projection for editors/TS/foreign tools only and is not runtime truth; runtime always resolves from the graph-backed PnP view.
...
- Writing project `node_modules` by default.
...
impl node_resolver::NpmPackageFolderResolver for DenoNodePnpBridge { ... }
impl node_resolver::InNpmPackageChecker for DenoNodePnpBridge { ... }
impl deno_node::NodeRequireLoader for DenoNodePnpBridge { ... }
```
...
2. Introduce `DenoNodePnpBridge` over `ResolutionGraph`, `Cache`, and `UnpackedStore`.
3. Map first-party referrers and unpacked-store referrers back to graph package identity.
4. Return real package folder paths from `UnpackedStore::ensure(...)` for Deno's resolver.
...
- dependency CJS runs from the cache/unpacked store with no project `node_modules`;
...
- `__dirname` / `__filename` point at local or unpacked-store real paths;
...
LOAD-004 should land after RT-007 has registered Deno Node built-ins. It replaces the temporary native-CJS shim if present. Rollback is to the previous meow-owned CJS shim only as an emergency compatibility stopgap; the accepted architecture remains Deno-owned Node semantics.
