...
**Definition of "shipped" (MVP, CANON §22):** a developer can `meow run` a small TypeScript **ESM** web service that uses dependencies, with default `meow install` materializing a strict `node_modules` projection, deterministic lockfile-pinned resolution, type stripping (no emitted JS), and identical behavior across two machines on the same `meow` version + lockfile.
**Mode:** `incubating` (flip to graduated only per CONSTITUTION A8).
**Stack:** Rust — one **Cargo workspace (monorepo)**, subsystems are crates; embeds V8 via upstream `deno_core`/`rusty_v8`; Node compatibility via upstream Deno extension crates (`deno_node`, `deno_napi`, `deno_crypto`, `deno_fetch`, `deno_net`); parse/transform via Oxc; bundling via Rolldown. **Prod target:** none yet.
...
| `LOAD` | Module loader & resolution | ESM resolution, PnP package placement, Deno-Node bridge over `ResolutionGraph` + `UnpackedStore`, lockfile-at-load; the one shared package graph | omp | I-1, I-2, I-5, I-7 |
...
| `TOOL` | Lint · format · bundle | shared `crates/tool` (`meow-tool`) for fmt/lint/bundle CLI slices over the shared graph | omp | I-1, I-2, I-10, I-11 |
...
| `LSP` | Language server | shared parser/resolver (`ResolutionGraph` runtime truth), `meow:*` types, editor-only shadow `node_modules` symlink map, IDE diagnostics | claude | I-1, I-5, I-9 |
...
| `CFG` | Configuration | `defineMeow` runtime/tooling/policy config, `package.json` authority integration, shadow `.meow/tsconfig.json` generation, `meow sync` | claude | I-1, I-9 |
...
> ⚠ **Amendments 001/002/003 (CANON) re-order this.** Drop-in compatibility + Tier-1 UX now lead. A new **P2.5 · Drop-In Core** is inserted before P3 (it pulls the former Phase-5 node-compat + CJS work forward), and Amendment 003 changes the implementation strategy: use upstream Deno Node/N-API crates and make `meow` the PnP/cache bridge. The dependency order below is preserved for history; the amendments override *priority and implementation strategy*, not the dependency facts.
...
- **P1 · Minimal Runtime (MVP)** — `meow run`; ESM loader; TC39 type stripping (erasable-only); `strict-web` "Stateless Edge" globals (`fetch`/`Response`/`Headers`/`URL`/`crypto.subtle`/`TextEncoder`/`AbortController`/`Blob`/`FormData`/`setTimeout`); early `meow:*` + generated types; historical first-party CJS refusal guard (superseded by P2.5).
  - *Exit:* a small TS web server runs with no emitted JS; behavior is identical across two machines on the same version+lockfile; the P1 CJS refusal was an MVP guard, not the current target. Gates: `determinism` (happy path), `compat` (historical CJS refusal), `strip-fidelity`, `types-fresh`.
- **P2 · Packages & Resolution** — content-addressed cache; deterministic `meow.lock.jsonl`; default filesystem-first materialized install projection (symlink-backed, no-hoist `node_modules`) with deprecated in-memory mode only via `--mode pnp`; npm registry resolution; integrity checks; `meow install`; `meow why-dep`.
  - *Exit:* a project installs and runs dependencies with default materialized `node_modules` via strict no-hoist layout; the same lockfile reproduces the same graph; runtime and LSP resolve packages identically. `.meow/deps/` is kept as a flat editor projection with collision recording for multi-version cases. Gates: `resolver-parity`, `lockfile-integrity`.
- **P2.5 · Drop-In Core (Amendments 001/002/003 — the adoption spine, Tier-1)** — `package.json` becomes the dependency/scripts/workspaces authority (`meow` reads it; `meow add`/`remove` mutate it; the `meow.config.dependencies` field + config→package.json generation are retired); Node compatibility is delegated to upstream Deno crates (`deno_node`, `deno_napi`, `deno_crypto`, `deno_fetch`, `deno_net`); the meow bridge feeds Deno real immutable `UnpackedStore` package folders selected by the PnP `ResolutionGraph`; `meow run <script>` / `meow <script>` / `meow dev`. Default mode flips to drop-in; `strict-web` becomes opt-in.
  - *Exit (the 5-minute test):* `cd existing-project && meow install && meow run dev` runs a stock npm/Bun app with **zero file edits**, including Node built-ins, native CommonJS cycles/dynamic require, and N-API `.node` addons loaded from the cache-backed unpacked store. Gates: `drop-in` (real-project corpus installs + runs unmodified), `resolver-parity` (same package graph selection at runtime/LSP/tooling), `determinism` (opt-in/frozen).
...
- **P5 · Node Compatibility & Legacy residuals** — compatibility corpus expansion, legacy monkey-patching edge cases, hardened materialized install, and any Deno gaps that need upstream tracking or meow-specific bridges.
  - *Exit:* remaining long-tail real projects either run or fail with precise upstream/meow gap labels; no hand-authored Node-polyfill fork or synthetic CJS wrapper has reappeared. Gates: `compat`, `drop-in`, `honesty`.
...
- Runtime truth is `ResolutionGraph` (exact per referrer); `UnpackedStore` backs real package paths for Deno Node/N-API; `.meow/deps/` is the symlink-first editor/TS/LSP shadow and may flatten multi-version names with collision records; deprecated `--mode pnp` keeps install virtual when explicitly requested, while `--vendor` remains the full copy escape hatch for offline/frozen deploys.
...
- Biggest redesign risks (CANON §24): `tsgo` maturity for `CHK` (fallback `tsc`); Deno Node bridge depth for PnP/CJS/N-API (feed real unpacked paths, do not force `meow-cache://` through path-oriented APIs); V8/Deno crate version churn for `DIST`/`RT` (track upstream, never fork).
...
- `LOAD-002` — Historical first-party CJS refusal guard (P1); superseded by P2.5's Deno-owned native CJS bridge (I-2/I-11).
- `LOAD-003` — Full Node resolution algorithm (conditions, `exports`/`imports`, self-references) shared by runtime + LSP (I-5).
- `CFG-002` — Legacy root `package.json` projection from `meow.config.ts`; to be retired by CFG-003's `package.json` authority (I-9).

**Later phases (sketch — Amendment 001/002/003 priority order)**
- **P2.5 · Drop-In Core (NOW — Tier 1, the adoption spine):** `CFG-003` **package.json = dependency authority** (read deps from `package.json`; `meow add`/`remove` mutate it; retire the `meow.config.dependencies` field + the `CFG-002` config→package.json *generation*); `RUN-001` `meow run <script>` / `meow <script>` / `meow dev` (package.json scripts runner); `RT-007` becomes **Deno Node built-ins harvest** (register upstream `deno_node` built-ins/globals, remove manual `js/node` ownership); `LOAD-004` becomes **PnP→Deno CJS bridge** (feed Deno's require loader real unpacked cache paths selected by `ResolutionGraph`, not synthetic ESM wrappers); add an N-API slice wiring `deno_napi` so `.node` addons load from extracted cache files.
- P3 (Parse-Once Toolchain — backlog drafted, reconciled to current slice): `TOOL-001` formatter, `TOOL-002` OXC-native linter, `TOOL-003` rolldown bundling skeleton (CLI + dep wiring only; PnP Resolver bridge finalized in a follow-up); `CHK-001` delegated `meow check`; `CHK-002` fast-preview linter.
...
- P5 (residual after Deno harvest): compatibility corpus expansion, legacy monkey-patching edge cases, hardened materialized install, and precise upstream-gap tracking.
