...
pub fn add_dependency(
    ...
    let deps = match dependencies_object_mut(&path, root_object, true)? {
        Some(deps) => deps,
...
pub fn remove_dependency(root: &Path, name: &PackageName) -> Result<PackageJson, ConfigError> {
    ...
    let root_object = root_object_mut(&path, &mut value)?;
    let remove_field = {
        let Some(deps) = dependencies_object_mut(&path, root_object, false)? else {
            return parse_package_json_value(&path, &value);
        };
...
fn dependencies_object_mut<'a>(
    ...
    if !root.contains_key("dependencies") {
        ...
        root.insert("dependencies".to_owned(), Value::Object(Map::new()));
    }

    match root.get_mut("dependencies") {
